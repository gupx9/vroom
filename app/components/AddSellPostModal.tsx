...
export default function AddSellPostModal({ onClose }: Props) {
  ...
  const handleSelect = (id: string, type: ItemType, label: string, imageData: string | null) => {
    setSelected({ id, type, label, imageData });
    setPrice('');
    setPriceError('');
    setSubmitError('');
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
                  ...
                  <div className="space-y-2">
                    {cars.map((car) => (
                      <button
                        key={car.id}
                        type="button"
                        onClick={() => handleSelect(car.id, 'car', `${car.brand} ${car.carModel}`, car.imageData)}
                  ...
                  <div className="space-y-2">
                    {dioramas.map((d) => (
                      <button
                        key={d.id}
                        type="button"
                        onClick={() => handleSelect(d.id, 'diorama', d.description, d.imageData)}
                        className={`w-full flex items-center gap-3 p-3 rounded-xl border text-left transition-all ${
                          selected?.id === d.id
                            ? 'border-red-500 bg-red-950/30'
                            : 'border-zinc-800 bg-zinc-800/40 hover:border-zinc-600'
        ...
        {/* Price input and submit — only shown when something is selected */}
        {selected && (
          <form onSubmit={handleSubmit} className="px-6 py-4 border-t border-zinc-800 space-y-3">
            <div>
              <label className="block text-xs font-medium text-zinc-400 mb-1.5">
                Selling Price (৳) for <span className="text-zinc-200">{selected.label}</span>
              </label>
              <input
                type="number"
                min="1"
