      ...
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
          <div>
            ...
            <p className="text-zinc-400 text-xs mt-0.5">List one of your cars for auction</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {loading ? (
            <div className="py-16 text-center text-zinc-500 text-sm">Loading your cars…</div>
          ) : cars.length === 0 ? (
            <div className="py-16 text-center">
              <p className="text-zinc-400 font-medium">No eligible cars</p>
              <p className="text-zinc-600 text-sm mt-1">All your cars are already in active auctions or your garage is empty.</p>
            </div>
          ) : (
            <>
              {/* Car selector */}
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Select a car</p>
                <div className="space-y-2">
                  {cars.map((car) => (
                    <button
                      key={car.id}
                      type="button"
                      onClick={() => { setSelectedCar(car); setBidError(''); setSubmitError(''); }}
              ...
              {selectedCar && (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="border-t border-zinc-800 pt-4">
                    <p className="text-xs font-bold uppercase tracking-widest text-zinc-500 mb-3">Auction Settings</p>
                    ...
                    <div className="mb-3">
                      ...
                      <input
                        ...
                        step="any"
                        value={startingBid}
                        onChange={(e) => { setStartingBid(e.target.value); setBidError(''); setSubmitError(''); }}
                        ...
                        autoFocus
                      />
                      {bidError && <p className="mt-1 text-xs text-red-400">{bidError}</p>}
                    </div>
