  ...
  return (
    <div className={`text-center ${isUrgent && !ended ? "animate-pulse" : ""}`}>
      {ended ? (
        ...
        <p
          className={`text-3xl font-black font-mono tracking-widest ${isUrgent ? "text-red-400" : "text-white"}`}
        >
          {h > 0 ? `${pad(h)}:` : ""}
          {pad(m)}:{pad(s)}
        </p>
      ...
      <p className="text-[10px] uppercase tracking-widest text-zinc-600 mt-0.5">
        {ended ? "Auction over" : "Time remaining"}
      </p>
    </div>
  );
}
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-4">
        <div>
          <h2 className="text-2xl font-bold text-white">
            {car.brand} <span className="text-zinc-300">{car.carModel}</span>
          </h2>
...
function InfoField({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-zinc-800/50 rounded-lg p-3">
      <p className="text-[10px] uppercase tracking-widest text-zinc-600 mb-0.5">
        {label}
      </p>
        ...
        {auction.winner ? (
          <div className="bg-gradient-to-r from-amber-950/60 to-yellow-950/40 border border-amber-700/50 rounded-2xl p-6 text-center space-y-1">
            ...
            <p className="text-zinc-300 text-sm">
              <Link
                href={`/profile/${auction.winner.username}`}
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 text-center">
            ...
            <p className="text-zinc-600 text-sm mt-1">
              The car was returned to the owner's garage.
            </p>
          </div>
        )}
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 text-center">
          ...
          {timerExpired && !isFinalized && (
            <p className="text-xs text-zinc-600 mt-2">
              Finalizing auction automatically...
            </p>
          )}
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
          ...
          {topBid ? (
            <div className="flex items-center justify-between">
              <Link
                href={`/profile/${topBid.bidder.username}`}
        ...
        return (
          ...
          >
            ...
            >
              @{bid.bidder.username}
              {isMe && (
                <span className="ml-1.5 text-[10px] text-red-400/70 font-normal">
                  (you)
                </span>
              )}
            </Link>
            <span
              className={`text-sm font-bold shrink-0 ${isWinner ? "text-amber-400" : "text-zinc-300"}`}
            >
